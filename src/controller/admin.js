...
module.exports = class extends Base {
  ...
  async saleAdminListAction() {
    let data = await this.model('user_admin').select()
    return this.success(data)
  }
  async adminDetailAction() {
    let id = this.post('id')
    let info = await this.model('admin')
      .where({
        id: id,
      })
      .find()
  ...
  async senderInfoAction() {
    let info = await this.model('settings')
      .where({
        id: 1,
      })
      .find()
